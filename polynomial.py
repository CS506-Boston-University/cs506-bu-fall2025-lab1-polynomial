...
class Sub:
    ...
    def __repr__(self):
        # Parenthesize Add/Sub to preserve grouping
...
class Div:
    ...
    def __repr__(self):
        # Keep your stricter rule: parens for Add/Sub/Mul/Div on the right
...
try:
    ...
    print(f"Evaluation for X=4: {result.i}")
except Exception:
    print("❌ Evaluation test failed - evaluate methods not implemented yet")

try:
    original_result = poly.evaluate(2)
    print(f"Original polynomial evaluation for X=2: {original_result.i}")
except Exception:
